...
def azimute(A,B):
    # Calculation of Azimuths between two points (Vector AB origin A end B)
    if ((B.x()-A.x())>=0 and (B.y()-A.y())>0): #1º quadrant
        AzAB=arctan((B.x()-A.x())/(B.y()-A.y()))
        AzBA=AzAB+pi
    elif ((B.x()-A.x())>0 and(B.y()-A.y())<0): #2º quadrant
        AzAB=pi+arctan((B.x()-A.x())/(B.y()-A.y()))
        AzBA=AzAB+pi
    elif ((B.x()-A.x())<=0 and(B.y()-A.y())<0): #3º quadrant
        AzAB=arctan((B.x()-A.x())/(B.y()-A.y()))+pi
        AzBA=AzAB-pi
    elif ((B.x()-A.x())<0 and(B.y()-A.y())>0): #4º quadrant
        AzAB=2*pi+arctan((B.x()-A.x())/(B.y()-A.y()))
        AzBA=AzAB+pi
    elif ((B.x()-A.x())>0 and(B.y()-A.y())==0): # on the positive x axis (90º)
        AzAB=pi/2
        AzBA=1.5*pi
    else: # ((B.x()-A.x())<0 and(B.y()-A.y())==0) # on the negative x axis (270º)
        AzAB=1.5*pi
        AzBA=pi/2
    # Correction of angles for the range 0 to 2pi
...
def DifAz(Az_ini, Az_fim):
    # Difference (angle) between azimuths
    dAz = Az_fim - Az_ini
    if dAz < 0:
...
# Conversion from geocentric to geodesic coordinates
def geoc2geod(X, Y, Z, a, f):
    b = a*(1-f)
    e2 = f*(2-f) # first eccentricity
    e2_2 = e2/(1-e2) # second eccentricity
...
# Conversion from Topocentric to Geocentric Coordinates
